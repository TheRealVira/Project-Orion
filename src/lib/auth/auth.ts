...
export function getSessionByToken(token: string): (Session & { user: User }) | null {
  ...
  const stmt = db.prepare(`
    SELECT 
      ...
      u.city as user_city, u.country as user_country, u.timezone as user_timezone,
      u.latitude as user_latitude, u.longitude as user_longitude,
      u.locationSource as user_locationSource, u.locationUpdatedAt as user_locationUpdatedAt
    FROM sessions s
  ...
  return {
    ...
    user: {
      ...
      locationSource: row.user_locationSource,
      locationUpdatedAt: row.user_locationUpdatedAt,
    },
  };
}
...
export function updateUser(id: string, updates: Partial<User>): User | null {
  ...
  const allowedFields = [
    'name', 'email', 'phone', 'role', 'avatarUrl', 'isActive',
    'city', 'country', 'timezone', 'latitude', 'longitude', 
    'locationSource', 'locationUpdatedAt'
  ];
